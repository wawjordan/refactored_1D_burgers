function [Esoln,EsolnIC,soln,OUT,S,stencil] = ETEIC_solver_alg1_SDIRK(grid,Esoln,EsolnIC,soln,S)
%%
for fakeloop = 1:1
...
S.isSDIRK2ETE = (isa(S.ETEintegrator,'SDIRK2ETE_type'));
S.isSDIRK2ETEIC = (isa(S.ETEintegratorIC,'SDIRK2ETE_type'));
if (S.isBDF2)
...
for i = S.count:S.stencil_size % march forward with ETE solution
    ...
    if (S.isSDIRK2ETE)
        % stage 1
        RU = ss_residual(U,grid.dx,S.nu,grid.N);
        [u1,dudt1] = S.LS_T.eval(stencil,time,1); % Reconstruction from primal
        TE1 = ss_residual_cont(S,S.LS_S,u1);
        TE1 = TE1 + dudt1;
        RHS1 = @(e) S.ETE_RHS(U,e,RU,TE1);
        LHS1 = @(e) S.ETE_LHS(U,e);
        e_old = Esoln.U;
        [e_new,~,S,S.ETEintegrator] = S.ETEintegrator.step1(...
            e_old, S, RHS1, LHS1, L_BC1, L_BC2, R_BC1, R_BC2 );
%         Esoln.U = e_new;
        % stage 2
        alpha = (2-sqrt(2))/2;
        time2 = time + alpha*S.dt;
        [u2,dudt2] = S.LS_T.eval(stencil,time2,1); % Reconstruction from primal
        RU = ss_residual(u2,grid.dx,S.nu,grid.N);
        
        TE2 = ss_residual_cont(S,S.LS_S,u2);
        TE2 = TE2 + dudt2;
        RHS2 = @(e) S.ETE_RHS(u2,e,RU,TE2);
        LHS2 = @(e) S.ETE_LHS(u2,e);
        [e_new,resnorm,S,S.ETEintegrator] = S.ETEintegrator.step2(...
            e_old, S, RHS2, LHS2, L_BC1, L_BC2, R_BC1, R_BC2 );
        Esoln.U = e_new;
    else
        RU = ss_residual(U,grid.dx,S.nu,grid.N);
        [u,dudt] = S.LS_T.eval(stencil,time,1); % Reconstruction from primal
...
for j = 1:S.Niters   % iterative correction steps
    ...
    for i = S.count:S.stencil_size % march forward in time with ETE solution
        if (S.isSDIRK2ETEIC)
            ...
            time = stencil.t(i);
            [u1,dudt1] = S.LS_T.eval(stencil,time,2);
            TE1 = ss_residual_cont(S,S.LS_S,u1);
            TE1 = TE1 + dudt1;
            ...
            e_old = Esoln.U;
            %% solve ETE
            [~,~,S,S.ETEintegratorIC] = S.ETEintegratorIC.step1(...
                e_old, S, RHS1, LHS1, L_BC1, L_BC2, R_BC1, R_BC2 );

            % stage 2
            alpha = (2-sqrt(2))/2;
            time2 = time + alpha*S.dt;
            [u2,dudt2] = S.LS_T.eval(stencil,time2,2); % Reconstruction from primal
            %% set BCS
            L_BC1 = S.L_BC1;
            L_BC2 = S.L_BC2;
            R_BC1 = @(u,n) S.R_BC1(u,n,0); % dirichlet BC
            ...
            TE2 = TE2 + dudt2;
            %% Define system of equations
            RHS1 = @(e) S.ETE_RHS(u2,e,RU,TE2);
            LHS1 = @(e) S.ETE_LHS(u2,e);
            %% set initial guess
            e_old = Esoln.U;
            %% solve ETE
            [e_new,resnorm,S,S.ETEintegratorIC] = S.ETEintegratorIC.step2(...
                e_old, S, RHS1, LHS1, L_BC1, L_BC2, R_BC1, R_BC2 );
            Esoln.U = e_new;
        else
            U = stencil.U(:,i,2); % corrected solution stencil
