function [Esoln,EsolnIC,soln,OUT,S,stencil] = ete_solver_w_IC_extrap(grid,Esoln,EsolnIC,soln,S)
%{
S   - input structure
...
S = dt_options(S);
% 1st level - primal
% 2nd level - corrected solutions from ETE
% 3rd level - corrected solutions from IC

stencil = soln_stencil( S.stencil_size, S.N, 3 );
...
for j = 1:1   % iterative correction steps
    ...
    for i = 2:S.stencil_size % march forward in time with ETE solution
        % 3rd level - corrected solutions from previous IC
        U = stencil.U(:,i,2);
        %% set BCS
        L_BC1 = S.L_BC1;
        L_BC2 = S.L_BC2;
...
%%
for j = 2:S.Niters   % iterative correction steps
    %% Make array for estimated error
    estError = zeros(grid.N,S.stencil_size);
    %% zero out error estimates
    S.ETEintegratorIC.um2 = zeros(grid.N,1);
    ...
    for i = 2:S.stencil_size % march forward in time with ETE solution
        ...
        %% Estimate truncation error
        time = stencil.t(i);
        [u,dudt] = S.LS_T.eval(stencil,time,3);
        TE = ss_residual_cont(S,S.LS_S,u);
        TE = TE + dudt;
        %% Define system of equations
        RHS = @(e) S.ETE_RHS(U,e,RU,TE);
    ...
    %% apply correction to stencil
    stencil.U(:,:,3) = stencil.U(:,:,3) - estError;
    %% output info
    for i = 2:S.stencil_size
%         k = min(j+1,stencil.L-1);
%         UE = stencil.U(:,i,k); % corrected solution stencil
%         if (j<S.Niters) % propagate correction to next iteration(?)
%             stencil.U(:,:,k+1) = stencil.U(:,:,k);
%         end
        UE = stencil.U(:,i,3); % corrected solution stencil
        OUT = output_ete_iter_stuff(OUT,S,resnorm,UE,i,j);
    end
%     for i = 2:S.stencil_size
%         UE = stencil.U(:,i,2); % corrected solution stencil
...
for j = 1:1%S.Niters   % iterative correction steps
    ...
    R_BC2 = @(u,n) S.R_BC2(u,n,0); % dirichlet BC
    RU = ss_residual(U,grid.dx,S.nu,grid.N);
    % 3rd level - extrapolated corrected solutions from previous IC
    [u,dudt] = S.LS_T.eval(stencil,time,1);
%     [u,dudt] = S.LS_T.eval(stencil,time,3);
    TE = ss_residual_cont(S,S.LS_S,u);
    TE = TE + dudt;
...
%     UE = stencil.U(:,i,3) - e_new; % corrected solution
    stencil.U(:,i,3) = U - e_new; % update stencil with corrected solution
...
% %     e_old = stencil.U(:,i-1,1)-stencil.U(:,i-1,2);
% %     time = stencil.t(i);
% %     U = stencil.U(:,i,1); % corrected solution stencil
% %     L_BC1 = S.L_BC1;
% %     L_BC2 = S.L_BC2;
...
% % %     stencil.U(:,i,3) = stencil.U(:,i,2); % update stencil with corrected solution
% %     OUT = output_ete_iter_stuff(OUT,S,resnorm,UE,count,j);
% % end
% % 1st step is initial condition (error is 0)
% i = stencil.queue_length;
...
function [Esoln,stencil,OUT,S] = step_ete(grid,Esoln,stencil,OUT,S,count)
    ...
    stencil.U(:,stencil.queue_length,3) = stencil.U(:,stencil.queue_length,2);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [soln,stencil,OUT,S] = step_primal(grid,soln,stencil,OUT,S,count)
    ...
    [u_new,resnorm,S,S.integrator] = S.integrator.step(...
                      u_old, S, S.RHS, S.LHS, L_BC1, L_BC2, R_BC1, R_BC2 );
    soln.U = u_new;
%     [U1,~] = S.LS_T.eval(stencil,time,3); % Extrapolate corrected solution for next step
    stencil = stencil.push(soln.U,time); % update stencil
%     stencil.U(:,stencil.queue_length,3) = U1; % Extrapolated corrected solution
    soln.E = soln.U - Uex;
    OUT = output_primal_stuff(OUT,S,soln.U,Uex,soln.E,resnorm,count);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
