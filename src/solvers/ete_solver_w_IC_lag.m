function [Esoln,EsolnIC,soln,OUT,S,stencil] = ete_solver_w_IC_lag(grid,Esoln,EsolnIC,soln,S)
%{
S   - input structure
...
OUT.t           = (S.t0:S.dt:S.tf+S.stencil_size*S.dt)';
...
[Esoln,stencil,OUT,S] = populate_ete_lag(grid,Esoln,stencil,OUT,S);
...
while solving
    ...
    [Esoln,stencil,OUT,S] = step_ete_lag(grid,Esoln,stencil,OUT,S,count);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Esoln,stencil,OUT,S] = populate_ete_lag(grid,Esoln,stencil,OUT,S)
% 1st step is initial condition (error is 0)
M = (S.stencil_size+1)/2;
for i = 2:M % march forward with ETE solution
    time = stencil.t(i);
    U = stencil.U(:,i,1);
    L_BC1 = S.L_BC1;
    L_BC2 = S.L_BC2;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Esoln,stencil,OUT,S] = populate_iterates_lag(grid,Esoln,stencil,OUT,S)
for j = 1:S.Niters   % iterative correction steps
    ...
    M = (S.stencil_size+1)/2;
    for i = 2:M % march forward in time with ETE solution
...
%     S.ETEintegratorIC.um1 = S.ETEintegratorIC.um2;
%     e_old = S.ETEintegratorIC.um2;
...
for j = 1:S.Niters   % iterative correction steps
    ...
    e_old = stencil.U(:,i,1)-stencil.U(:,i,j+1);
    time = stencil.t(i);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Esoln,stencil,OUT,S] = step_ete_lag(grid,Esoln,stencil,OUT,S,count)
    M = (S.stencil_size+1)/2;
    time = stencil.t(M);
    U = stencil.U(:,M);
    L_BC1 = S.L_BC1;
