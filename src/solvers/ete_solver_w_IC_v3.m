function [Esoln,EsolnIC,soln,OUT,S,stencil] = ete_solver_w_IC_v3(grid,Esoln,EsolnIC,soln,S)
%{
S   - input structure
...
for j = 1:S.Niters   % iterative correction steps
    ...
    for i = 2:S.stencil_size % march forward in time with ETE solution
        U = stencil.U(:,i,2); % corrected solution stencil from initial ETE solve
        %% set BCS
        L_BC1 = S.L_BC1;
        L_BC2 = S.L_BC2;
...
for j = 1:S.Niters   % iterative correction steps
    ...
    DAT = stencil2tecplot_alg1_tvi(DAT,S,grid,stencil,S.xloc_out,j+1,false);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    UE = U - S.relax*e_new; % corrected solution
...
function [soln,stencil,OUT,S] = step_primal(grid,soln,stencil,OUT,S,count)
    ...
    [u_new,resnorm,S,S.integrator] = S.integrator.step(...
                      u_old, S, S.RHS, S.LHS, L_BC1, L_BC2, R_BC1, R_BC2 );
    soln.U = u_new;
%     [U1,~] = S.LS_T.eval(stencil,time,2);
    stencil = stencil.push(soln.U,time); % update stencil
%     [U2,~] = S.LS_T.eval(stencil,time,2);
%     stencil.U(:,stencil.queue_length,2) = U1; % Extrapolate corrected solution for next step
    soln.E = soln.U - Uex;
    OUT = output_primal_stuff(OUT,S,soln.U,Uex,soln.E,resnorm,count);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
