function [SE] = parse_IC_OOA_local_error(filename,h)
...
SE.dataFmt='%g';
SE.variables{1} = 'x';
SE.variables{2} = 'Exact Error';
SE.variables{3} = 'Richardson Extrapolation';
SE.variables{4} = 'Estimated Error (ETE)';
for j = 2:N
    k = j + 3;
    if j == 2
...
for jj = 1:N
    tmp1(:,jj) = interp1(OUT.Local_Error_P(h).x(:),OUT.Local_Error_E(h).e{end,jj},OUT.Local_Error_P(end).x(:));
end

SE.DATA(h-1).dat = [...
    OUT.Local_Error_P(end).x(:),...
    interp1(OUT.Local_Error_P(h).x(:),OUT.Local_Error_P(h).E{end}(:),OUT.Local_Error_P(end).x(:)),...
    E_RICH,...
    tmp1,...
    ];
end
